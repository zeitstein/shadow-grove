(ns shadow.grove.eql-query-brimm-test
  (:require
...
(deftest test-query-join-ident-not-found
  (let [env {}
    ...
    (is (= {ident {:join-key ident :db/not-found true}} res))))
...
(deftest test-query-coll-ident-not-found
  (let [env {}
    ...
    (is (= {:foo [{:join-key ident :db/not-found true}]} res))))
